...
- **推奨**: Phoenix の認証（[phx.gen.auth](https://hexdocs.pm/phoenix/authentication.html) 等）でユーザー・セッションを管理し、`connect/3` で `user_id`（および必要なら `username`）を `socket.assigns` に載せる。
- 全 Channel で `socket.assigns.user_id` を前提にできるため、フレンド・メッセージ・ルーム参加の権限チェックがしやすい。

```elixir
# イメージ: connect で user を assigns に
def connect(params, socket, _connect_info) do
  case verify_user(params) do
    {:ok, user} -> {:ok, assign(socket, :user_id, user.id) |> assign(:user, user)}
    _ -> :error
  end
end
```
...
- **配送**: 送信時に、受信者の Channel（例: `"user:" <> to_user_id`）に `push socket, "new_message", payload` する。未接続の場合は DB に残し、次回接続時に履歴 API または Channel の `join` 返却で渡す。
...
|------------|------|---------------------------|------------------|
| **少人数（2〜8 人）** | WebRTC P2P（メッシュ） | マッチング・シグナリングのみ。ゲームの高頻度データはサーバーを流さない。 | 各クライアントが対等に P2P で通信 |
...
|------|------|-----------------|---------------------------|
| **ホスト 1 人 + 他は P2P** | 1 プレイヤーをホストとし、他プレイヤーはホストのクライアントに接続。ゲームのループ・判定はホスト側で行う。 | マッチング・ルーム一覧・「誰がホストか」の通知のみ。ゲームデータは流さない。 | Engine（GameLoop + GameWorld）をホストのクライアントで動かす。Phoenix はルーム作成・参加・WebRTC シグナリングのみ。 |
