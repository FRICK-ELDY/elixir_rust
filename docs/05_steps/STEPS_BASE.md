...
**補足**: Elixir（GenServer）は高精度な 60 Hz タイミングが苦手なため、本ステップで一度 GenServer ベースのループを実装したうえで、のちに **Step 41（GameLoop Rust 移行・高精度 60 Hz）** にて Rust 側へ移行する。ここでは「動くループ」の土台として GenServer 版を扱う。
