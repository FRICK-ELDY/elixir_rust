...
2. [Step 48: 3D レンダリング基盤](#2-step-48-3d-レンダリング基盤)
3. [Step 49: 3D カメラ・三人称視点](#3-step-49-3d-カメラ三人称視点)
4. [Step 50: 3D メッシュ描画・アトラス流用](#4-step-50-3d-メッシュ描画アトラス流用)
5. [Step 51: 三人称プレイヤー制御](#5-step-51-三人称プレイヤー制御)
6. [Step 52: 射撃・弾丸・レイキャスト](#6-step-52-射撃弾丸レイキャスト)
7. [Step 53: 敵の 3D スポーン・AI・衝突](#7-step-53-敵の-3d-スポーンai衝突)
8. [Step 54: UI・アセット流用・ポリッシュ](#8-step-54-uiアセット流用ポリッシュ)
...
Step 49: 3D カメラ・三人称視点
  └ View/Projection 行列・カメラ追従

Step 50: 3D メッシュ描画・アトラス流用
  └ 既存スプライトアトラスを 3D テクスチャに・プレイヤー/敵のメッシュ

Step 51: 三人称プレイヤー制御
  └ 移動・カメラ追従・照準（マウス/キーボード）

Step 52: 射撃・弾丸・レイキャスト
  └ FPS 武器・ヒット判定

Step 53: 敵の 3D スポーン・AI・衝突
  └ 敵配置・追跡 AI・ダメージ・経験値

Step 54: UI・アセット流用・ポリッシュ
...
- カメラは仮で固定（Step 49 で View/Projection を本実装）
...
- 俯角: やや上から見下ろす角度。Step 51 でマウス Y と連動させる
...
- 弾丸リストと敵リストで距離判定（球 vs 点 または 球 vs 球）。ヒット時は弾を消し、敵にダメージを加える（Step 53 で敵 HP と連携）

### 6.4 確認ポイント

- [ ] クリック（またはキー）で弾が発射され、飛んでいく
- [ ] 敵に当たると弾が消え、敵にダメージが入る（Step 53 と合わせて）

---

## 7. Step 53: 敵の 3D スポーン・AI・衝突

### 7.1 目標

- **敵の 3D 配置**: プレイヤー周囲に敵をスポーン。既存アトラスの敵スプライトを 3D で表示
- **AI**: プレイヤーに向かって移動（Chase）。既存 2D の Chase AI を XZ 平面（または 3D 空間）に拡張
- **衝突**: 敵がプレイヤーに接触するとプレイヤーにダメージ。弾が敵に当たると敵にダメージ・撃破で経験値（Step 54 でドロップ表示も可）
...
- 弾 vs 敵: Step 52 で実装したヒットで HP 減少。0 以下で撃破・経験値（とアトラス上のジェム等をドロップ表示）
...
**前提**: Step 1〜40 の汎用化（Game インターフェース・ゲーム分離・アセット管理）が済んでいることが望ましい。Step 41〜47 は 2D サバイバー向けのため、3D は並行して Step 48 から開始可能。
...
| [STEPS_ALL.md](./STEPS_ALL.md) | 全ステップ一覧（Step 48〜 の概要） |
| [STEPS_EXTENSION.md](./STEPS_EXTENSION.md) | Step 41〜47（2D 拡張） |
