...
**所属**: [STEPS_ALL.md](./STEPS_ALL.md) 2章 ゲームエンジン応用 の 2.4 節。

> **据え置き**: 本節は当面保留とする。2.1〜2.3 の後に再検討する。実施順序は **2.1 Rust lib 分割・整理**（[STEPS_RUST_LIB.md](./STEPS_RUST_LIB.md)）→ **2.2 2D ゲームの固め** → **2.3 EOS 実装**（[EPIC_ONLINE_SERVICES.md](../06_system_design/EPIC_ONLINE_SERVICES.md)）の後に再検討する。
...
**前提**: 1章（1.1.1〜1.5.7）のいずれか完了済み（既存 2D サバイバーはそのまま維持）。3D 用の新規ゲームとして「三人称 FPS」を追加する。  
**アセット**: 既存のヴァンパイアサバイバーズ系スプライトアトラスを 3D 用テクスチャとして流用する（ビルボード・簡易メッシュのテクスチャ等）。

---

## 2.4 節 全体ロードマップ（2.4.1〜2.4.7）

| 項 | 目標 |
|----|------|
| 2.4.1 | 3D レンダリング基盤（wgpu で 3D パイプライン・深度バッファ・頂点/法線/UV） |
| 2.4.2 | 3D カメラ・三人称視点（View/Projection 行列・カメラ追従） |
| 2.4.3 | 3D メッシュ描画・アトラス流用 |
| 2.4.4 | 三人称プレイヤー制御（移動・カメラ追従・照準） |
| 2.4.5 | 射撃・弾丸・レイキャスト |
| 2.4.6 | 敵の 3D スポーン・AI・衝突 |
| 2.4.7 | UI・アセット流用・ポリッシュ |
...
2. [2.4.1 3D レンダリング基盤](#2-241-3d-レンダリング基盤)
3. [2.4.2 3D カメラ・三人称視点](#3-242-3d-カメラ三人称視点)
4. [2.4.3 3D メッシュ描画・アトラス流用](#4-243-3d-メッシュ描画アトラス流用)
5. [2.4.4 三人称プレイヤー制御](#5-244-三人称プレイヤー制御)
6. [2.4.5 射撃・弾丸・レイキャスト](#6-245-射撃弾丸レイキャスト)
7. [2.4.6 敵の 3D スポーン・AI・衝突](#7-246-敵の-3d-スポーンai衝突)
8. [2.4.7 UI・アセット流用・ポリッシュ](#8-247-uiアセット流用ポリッシュ)
...
- カメラは仮で固定（2.4.2 で View/Projection を本実装）
...
- 俯角: やや上から見下ろす角度。2.4.4 でマウス Y と連動させる
...
- **照準**: カメラの向き＝プレイヤーの「向き」として、射撃方向に使う（2.4.5 で利用）
...
- 弾丸リストと敵リストで距離判定（球 vs 点 または 球 vs 球）。ヒット時は弾を消し、敵にダメージを加える（2.4.6 で敵 HP と連携）

### 6.4 確認ポイント

- [ ] クリック（またはキー）で弾が発射され、飛んでいく
- [ ] 敵に当たると弾が消え、敵にダメージが入る（2.4.6 と合わせて）

---

## 7. 2.4.6 敵の 3D スポーン・AI・衝突

### 7.1 目標

- **敵の 3D 配置**: プレイヤー周囲に敵をスポーン。既存アトラスの敵スプライトを 3D で表示
- **AI**: プレイヤーに向かって移動（Chase）。既存 2D の Chase AI を XZ 平面（または 3D 空間）に拡張
- **衝突**: 敵がプレイヤーに接触するとプレイヤーにダメージ。弾が敵に当たると敵にダメージ・撃破で経験値（2.4.7 でドロップ表示も可）
...
- 弾 vs 敵: 2.4.5 で実装したヒットで HP 減少。0 以下で撃破・経験値（とアトラス上のジェム等をドロップ表示）
...
**前提**: 1.4（汎用化）が済んでいることが望ましい。1.5 は 2D サバイバー向けのため、3D は並行して 2.4.1 から開始可能。
