...
**所属**: [STEPS_ALL.md](../STEPS_ALL.md) 1章 エンジン構築 の 1.10 節。

> **据え置き**: 本節は当面保留とする。1.9 完了後に実施する。実施順序は **1.6 Rust lib 分割・整理**（[STEPS_RUST_LIB.md](./STEPS_RUST_LIB.md)）→ **1.7 2D ゲームの固め** → **1.8 EOS 実装**（[EPIC_ONLINE_SERVICES.md](../../06_system_design/EPIC_ONLINE_SERVICES.md)）の後に再検討し、1.9 完了後に実施する。

**目的**: [STEPS_3D.md](./STEPS_3D.md)（1.9）完了後、**Slot（transform 階層）＋ Component（カメラ・プレイヤー・敵・メッシュ等）を Elixir 側で管理**する設計に移行する。将来的なビジュアルエディタを見据え、シーングラフ・コンポーネント型・シリアライズ・Prefab の概念を実装に落とす。  
**前提**: 1.9 まで完了（3D レンダリング・三人称 FPS が動作）。本節で「正本を Elixir の Slot/Component に移し、Rust はシーンスナップショットを受け取って描画する」形にリファクタする。

---

## 1.10 節 全体ロードマップ（1.10.1〜1.10.7）

| 項 | 目標 |
|----|------|
| 1.10.1 | Slot（transform 階層）のデータ構造 |
| 1.10.2 | コンポーネント型・レジストリと Slot への付与 |
| 1.10.3 | ワールド行列計算・シーンスナップショット・Rust 連携 |
| 1.10.4 | 物理結果の Rust → Elixir 反映（Slot 同期） |
| 1.10.5 | シーンシリアライズ・保存/ロード |
| 1.10.6 | Prefab とインスタンス |
| 1.10.7 | ビジュアルエディタ向け基盤 |
...
2. [1.10.1: Slot（transform 階層）のデータ構造](#2-step-55-slottransform-階層のデータ構造)
3. [1.10.2: コンポーネント型・レジストリと Slot への付与](#3-step-56-コンポーネント型レジストリと-slot-への付与)
4. [1.10.3: ワールド行列計算・シーンスナップショット・Rust 連携](#4-step-57-ワールド行列計算シーンスナップショットrust-連携)
5. [1.10.4: 物理結果の Rust → Elixir 反映（Slot 同期）](#5-step-58-物理結果の-rust--elixir-反映slot-同期)
6. [1.10.5: シーンシリアライズ・保存/ロード](#6-step-59-シーンシリアライズ保存ロード)
7. [1.10.6: Prefab とインスタンス](#7-step-60-prefab-とインスタンス)
8. [1.10.7: ビジュアルエディタ向け基盤](#8-step-61-ビジュアルエディタ向け基盤)
...
## 2. 1.10.1 Slot（transform 階層）のデータ構造
...
| **Rust** | この項では未使用。1.10.3 でスナップショットを受け取る |
...
- `local_transform`: `%{position: {x, y, z}, rotation: {qx, qy, qz, qw} または {pitch, yaw, roll}, scale: {sx, sy, sz}}`。最小限は position のみでも可（rotation/scale は 1.10.3 で必要に応じて拡張）。
...
## 3. 1.10.2 コンポーネント型・レジストリと Slot への付与
...
| **Rust** | まだコンポーネントを直接参照しない。1.10.3 でスナップショットに「どの Slot に何が付いているか」が含まれる |
...
- **Camera**: FOV, near, far 等。この Slot の world transform が View の基準になる（1.10.3 で使用）。
- **Player**: HP, 装備等。入力でこの Slot を動かす対象。
- **Enemy**: HP, kind_id, AI 状態等。
- **Mesh**: アセット参照（mesh_id または path）、マテリアル参照等。この Slot の world transform でメッシュを描画する（1.10.3 で使用）。
...
## 4. 1.10.3 ワールド行列計算・シーンスナップショット・Rust 連携
...
- 3D レンダラーが「フレームごとに渡されるスナップショット」を入力とし、内部のカメラ・オブジェクト位置をスナップショットで上書きする。既存の GameWorld 内の位置は、1.10.4 で Slot に同期するまでは、Elixir が「プレイヤー Slot の world を GameWorld に書き戻す」などの橋渡しをしてもよい。
...
## 5. 1.10.4 物理結果の Rust → Elixir 反映（Slot 同期）
...
- [ ] 描画は 1.10.3 のスナップショットのまま、Slot の更新がそのまま画面に反映される
- [ ] 衝突・弾丸のヒット等が従来どおり動作する

---

## 6. 1.10.5 シーンシリアライズ・保存/ロード
...
- ファイルを読み、パースして Slot とコンポーネントを再構築。ID はそのまま復元するか、新規 ID を振り直すか方針を決める（Prefab インスタンス化と合わせて 1.10.6 で考慮してもよい）。
...
## 7. 1.10.6 Prefab とインスタンス

### 7.1 目標

- **Prefab** を「複数 Slot とコンポーネントからなる再利用可能なサブツリーの定義」として扱う。保存形式は 1.10.5 のシーン形式の部分木でよい。
...
- ファイルまたはメモリ上の「ルート Slot とその子孫」＋コンポーネント。形式は 1.10.5 のスロット列の部分集合。ルートの `parent_id` は未設定（インスタンス化時に親を指定する）。
...
**前提**: 1.9（STEPS_3D.md）完了。3D ゲームが既に動いている状態で、その「正本」を Slot/Component に移行する。
...
| ドキュメント | 用途 |
|-------------|------|
| [STEPS_3D.md](./STEPS_3D.md) | 1.9。本節の前提（3D レンダリング・三人称 FPS） |
