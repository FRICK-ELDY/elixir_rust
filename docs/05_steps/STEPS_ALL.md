...
| **拡張** | 40〜46 | GameLoop Rust 移行・マップ・セーブ・マルチ・デバッグ・GameEvents リネーム |
...
## 6. Step 41〜46: マップ・セーブ・マルチ・デバッグ・リネーム

| Step | 目標 | 備考 |
|------|------|------|
| 41 | GameLoop Rust 移行（高精度 60 Hz） | Step 7 のゲームループを Rust 側に移行（BEAM スケジューラ上では高精度なタイマー実行が保証されないため） |
| 42 | マップ・障害物システム | |
| 43 | セーブ・ロード | |
| 44 | マルチプレイ基盤（ルーム管理） | |
| 45 | デバッグ支援（NIF） | |
| 46 | GameLoop を GameEvent(s) にリネーム | Elixir 側の役割（frame_events 受信・フェーズ管理）に合わせてモジュール名を変更 |

**推奨順序**: 41 → 45 → 42 → 43 → 44 → 46（42 と 43 は並行可）  
...
- **41〜46**: 40 完了後。41 を優先すると他ステップの土台ができる。46 は 41 完了後であればいつでも実施可
...
| [STEPS_EXTENSION.md](./STEPS_EXTENSION.md) | Step 40〜46 の詳細 |
