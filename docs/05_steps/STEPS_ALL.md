...
| 7 | ゲームループ（GenServer 60 Hz） | ※Elixir は高精度 60 Hz が苦手なため、Step 41 で Rust に移行 |
| 8 | プレイヤー移動 | |
| 9 | 敵スポーン・追跡 AI | |
...
## 6. Step 41〜45: マップ・セーブ・マルチ・デバッグ

| Step | 目標 | 備考 |
|------|------|------|
| 41 | GameLoop Rust 移行（高精度 60 Hz） | Step 7 のゲームループを Rust 側に移行（Elixir は高精度タイミングが苦手なため） |
| 42 | マップ・障害物システム | |
| 43 | セーブ・ロード | |
| 44 | マルチプレイ基盤（ルーム管理） | |
| 45 | デバッグ支援（NIF） | |
...
- **41〜45**: 40 完了後。41 を優先すると他ステップの土台ができる
