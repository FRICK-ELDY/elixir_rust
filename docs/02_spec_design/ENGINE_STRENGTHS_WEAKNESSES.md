...
| 項目 | 説明 |
|------|------|
| **言語特性の役割分担** | Elixir: イベント駆動の司令塔・ウェーブ制御・シーン管理・EventBus。Rust: tick 駆動・物理演算・描画・音声。GameLoop は [Step 41](../05_steps/STEPS_MAP_SAVE_MULTI_DEBUG.md#3-step-41-gameloop-rust-移行) で Rust 移行予定。 |
...
| 課題 | 説明 | 対応状況 |
|------|------|----------|
| **60Hz ゲームループのジッター** | `Process.send_after/3` は Erlang スケジューラ依存で ±数 ms のジッターが発生。高精度タイマーが必要なゲーム（リズムゲーム等）には不向き。 | **対応予定**: [Step 41](../05_steps/STEPS_MAP_SAVE_MULTI_DEBUG.md#3-step-41-gameloop-rust-移行) で GameLoop を Rust に移行し、高精度タイマー（`std::time::Instant`）で固定間隔 tick を実現 |
...
| **マップ・タイル管理がない** | 無限平面のみ。障害物・壁・タイルマップの概念なし。Ghost（壁すり抜け）は現状未実装（マップ・障害物システムが必要、[Step 42](../05_steps/STEPS_MAP_SAVE_MULTI_DEBUG.md#4-step-42-マップ障害物システム) で対応予定）。 |
...
| 優先度 | 項目 | 備考 |
|--------|------|------|
| **高** | **GameLoop Rust 移行** | [Step 41](../05_steps/STEPS_MAP_SAVE_MULTI_DEBUG.md#3-step-41-gameloop-rust-移行) で実施。精度確保のため最優先 |
...
| [STEPS_MAP_SAVE_MULTI_DEBUG.md](../05_steps/STEPS_MAP_SAVE_MULTI_DEBUG.md) | Step 40（GameLoop Rust 移行）含む実装ロードマップ |
