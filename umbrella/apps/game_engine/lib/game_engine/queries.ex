# Path: umbrella/apps/game_engine/lib/game_engine/queries.ex
# Summary: Engine の query_light 境界（World を変更しない取得系操作を集約）
defmodule GameEngine.Queries do
  @moduledoc """
  Engine の `query_light` 境界。

  World を変更しない取得系操作をこのモジュールに集約する。
  ここには毎フレームで使う軽量 query を置く。
  """
  ...
  def get_save_snapshot_heavy(world_ref), do: NifBridge.get_save_snapshot(world_ref)
end
